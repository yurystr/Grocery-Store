...
class BasketTests: XCTestCase {
    ...
    func testTotalCount() {
        let basket = Basket()
        basket.update(count: 10, for: Product(identifier: "1", title: "Test", price: 0, unitOfMeasure: "Test"))
        XCTAssertEqual(basket.getTotalCount(), 10)
    }
    
    func testTotalSum() {
        let basket = Basket()
        let product1 = Product(identifier: "1", title: "Test 1", price: 1, unitOfMeasure: "Test 1")
        let product2 = Product(identifier: "2", title: "Test 2", price: 2, unitOfMeasure: "Test 2")
        basket.productList = ProductList(products: [product1, product2], currency: "Test")
